...
const CGFloat JCCPerceivedBrightnessInvalid = -1.f;

#if !TARGET_OS_WATCH
static UIColor * JCCGetFirstLineCompositionColorMaster(UIView *view, UIImage *image);

UIColor * JCCGetFirstLineCompositionColorWithView(UIView *view)
{
    return JCCGetFirstLineCompositionColorMaster(view, nil);
}

UIColor * JCCGetFirstLineCompositionColorWithImage(UIImage *image)
{
    return JCCGetFirstLineCompositionColorMaster(nil, image);
}

UIColor * JCCGetFirstLineCompositionColorMaster(UIView *view, UIImage *image)
{
    ...
    NSUInteger bytesPerRow = JCCByteAlignForCoreAnimation(bytesPerPixel * width);
...
{
    return [self jcc_RGBStringRepresentationWitAlpha:NO];
}
...
- (CGFloat)jcc_perceivedBrightness
{
    ...
    CGFloat brightness = JCCPerceivedBrightnessInvalid;
...
{
    CGFloat brightness = [self jcc_perceivedBrightness];
    if (brightness == JCCPerceivedBrightnessInvalid) {
        return NO;
    }
